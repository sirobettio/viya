...
FILES_LIST=($(ls /etc/init.d/sas-viya* | awk -F '/' '{print $NF}' | grep -vE "all-services|consul|vault|httpproxy|rabbitmq|sasdatasvrc|pgpoolc|cascontroller|cachelocator|cacheserver"))
...
for ((i=0; i<${#FILES_TO_PROCESS[@]}; i++)); 
   do
      if [ ${NUMBER_OF_PROCESS} == ${NUMBER_OF_PROCESS_LIMIT} ]
         then
            NUMBER_OF_PROCESS=0
            ...
            while [ ${DO_IT} == 1 ];
               do
                  ...
                  if [ ${CPUS_USAGE_CURRENT} -gt ${CPUS_USAGE_MAX} ];
                     then
							if [ ${DEBUG} == 1 ];
							   then 
      ...
      echo "execute command ${COMMAND_TYPE} for: ${i} ${NUMBER_OF_PROCESS} ${FILES_TO_PROCESS[$i]}: $(echo ${COMMAND} | sed 's/SVC/'${FILES_TO_PROCESS[$i]}'/g')"
